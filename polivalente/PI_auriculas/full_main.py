...
if __name__ == '__main__':
    ...
    session1 =""
    heart = pv.read(folder_files + session1 + "pos_heart.stl")
    pvtorso = pv.read(folder_files + session1 + "torso.stl")


    """To use her own heart"""
    centroid = heart.center
    mmheart = pv.read(folder_files + "real_heart.stl")
    mmheart = mmheart.decimate(.99)
    mmheart = mmheart.scale(1/1000, transform_all_input_vectors = True)
    mmheart = mmheart.scale(1/2, transform_all_input_vectors = True) # it's huge
    mmheart.rotate_x(90, mmheart.center, inplace = True)
    mmheart.translate(-np.array(mmheart.center)+np.array(centroid), inplace=True)
    mmheart = mmheart.connectivity(largest = False)
